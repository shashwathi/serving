...
func TestRevisionTimeout(t *testing.T) {
	...
	if err := v1test.WaitForRevisionState(clients.ServingClient, rev7s.Revision, v1test.IsRevisionReady, "RevisionIsReady"); err != nil {
	...
	if err := sendRequest(t, clients, rev7sURL, 3*time.Second, 5*time.Second, http.StatusOK); err != nil {
		t.Errorf("Did not fail request with sleep 4s/5s with revision timeout 7s: %v", err)
	}
